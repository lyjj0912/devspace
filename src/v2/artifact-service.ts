...
export class UniversalArtifactService {
  ...
  private async copy(input: UniversalArtifactInput): Promise<Record<string, unknown>> {
    ...
    try {
      ...
      return {
        operation: "copy",
        source: { target: sourceTarget ?? "local", path: sourcePath },
        destination: { target: destinationTarget ?? "local", path: destinationPath },
        size: exported.size,
        sha256: exported.sha256,
        published,
      };
  ...
  private resourceUri(record: ArtifactRecord): string {
    ...
    return configured
      ? `${configured.replace(/\/+$/, "")}/artifacts-next/${record.artifactId}?${query}`
