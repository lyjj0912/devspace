...
export const UNIVERSAL_BROKER_VERSION = "2.0.0-alpha.1";
...
const privilegeSchema = z.enum(["user", "admin"]);
...
const fsContract = {
  ...
  inputSchema: {
    ...
    privilege: privilegeSchema.optional(),
...
const execContract = {
  ...
  description:
    "Run a command locally or over SSH with optional user/admin privilege, PTY, and automatic conversion to a managed background process.",
  inputSchema: {
    target: z.string().min(1).optional(),
    contextId: z.string().min(1).optional(),
