...
test("administrator and environment-profile requests fail explicitly when unavailable", async (t) => {
  const fixture = await createFixture(t);
  await assert.rejects(
    fixture.execution.execute({
      target: "local",
    ...
    (error: unknown) => brokerCode(error) === "ADMIN_UNAVAILABLE",
  );
  await assert.rejects(
    fixture.execution.execute({
      target: "local",
...
test("remote cwd and sudo failures are classified before dispatch", async (t) => {
  const fixture = await createFixture(t);
  await assert.rejects(
    fixture.execution.execute({
      target: "fake",
...
async function createFixture(
  ...
  await writeFile(targetsPath, JSON.stringify({
    ...
    targets: {
      ...
      fake: sshTarget("fake", "unavailable"),
      "fake-admin": sshTarget("fake-admin", "sudo-n"),
      offline: sshTarget("offline", "unavailable"),
      unknown: sshTarget("unknown", "unavailable"),
    },
  }, null, 2));
...
function sshTarget(sshHost: string, adminMode: "unavailable" | "sudo-n") {
  return {
    displayName: sshHost,
    aliases: [sshHost],
    transport: "ssh",
    ...
    platform: "linux",
    shell: "sh",
    privilege: { user: true, admin: { mode: adminMode } },
  };
}
