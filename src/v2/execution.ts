...
export type ExecutionPrivilege = "user" | "admin";
...
export class UniversalExecutionPlane {
  ...
  async execute(input: ExecuteCommandInput): Promise<UniversalProcessSnapshot> {
    ...
    if (input.envProfile) {
      throw new UniversalBrokerError(
        ...
        "Environment profiles are reserved by the v2 exec contract but are not implemented in Phase 3.",
        {
          evidence: {
            phase: "phase-3",
    ...
    if (snapshot.state === "FAILED") {
      throw new UniversalBrokerError(
        (snapshot.errorCode as "TRANSPORT_UNAVAILABLE" | "PATH_NOT_FOUND" | "ADMIN_UNAVAILABLE")
          ?? "TRANSPORT_UNAVAILABLE",
        snapshot.errorMessage ?? "Command could not be dispatched.",
        {
  ...
  private async resolveExecution(input: ExecuteCommandInput): Promise<ResolvedExecution> {
    ...
    const privilege = input.privilege ?? "user";
    assertPrivilege(target, privilege);
  ...
  private finish(entry: ProcessEntry, code: number | null, signal?: string): void {
    ...
    if (entry.requestedSignal) {
      ...
      } else if (markers.adminRejected) {
        entry.state = "FAILED";
        entry.errorCode = "ADMIN_UNAVAILABLE";
        entry.errorMessage = `Non-interactive administrator privilege is unavailable on ${entry.targetId}.`;
  ...
  private list(cursor?: string, requestedLimit?: number): Record<string, unknown> {
...
class RemoteMarkerParser {
  ...
  readonly adminRejectedMarker: string;
  private remainder = "";
  dispatched = false;
  completed = false;
  cwdRejected = false;
  ...
  constructor(nonce: string) {
    ...
    this.adminRejectedMarker = `__DEVSPACE_ADMIN_REJECTED_${nonce}__`;
  }
  ...
  private sanitize(value: string, final: boolean): string {
    ...
    if (result.includes(this.adminRejectedMarker)) {
      this.adminRejected = true;
      result = result.replace(new RegExp(`${escapeRegExp(this.adminRejectedMarker)}\\r?\\n?`, "g"), "");
    }
...
async function localCommandSpec(
  ...
  const shell = localShellCommand(resolved.target, command);
  if (resolved.privilege === "admin") {
    if (resolved.target.privilege.admin.mode !== "sudo-n") {
      throw new UniversalBrokerError(
        "ADMIN_UNAVAILABLE",
        `Administrator execution is unavailable on target ${resolved.target.id}.`,
      );
    }
    return {
      executable: "sudo",
      args: ["-n", "--", shell.executable, ...shell.args],
      cwd: resolved.cwd,
...
async function remoteCommandSpec(input: {
  ...
  const { target, cwd, privilege } = input.resolved;
  if (!target.sshHost) {
    throw new UniversalBrokerError(
      "TARGET_OFFLINE",
  ...
  const remoteCommand = target.platform === "windows"
    ? windowsRemoteCommand(target, cwd, input.command, privilege, markers)
    : posixRemoteCommand(target, cwd, input.command, privilege, markers);
  return {
    ...
    args: [
      ...
      "-o", "ControlPersist=60",
      "-o", `ControlPath=${join(input.sshControlDir, "%C")}`,
      "-o", "LogLevel=ERROR",
      target.sshHost,
      remoteCommand,
    ],
...
function posixRemoteCommand(
  ...
  const runner = privilege === "admin"
    ? `sudo -n -- ${shell} -lc ${shellQuote(command)}`
    : `${shell} -lc ${shellQuote(command)}`;
  const adminCheck = privilege === "admin"
    ? `if ! sudo -n true >/dev/null 2>&1; then printf '%s\\n' ${shellQuote(markers.adminRejectedMarker)} >&2; exit 47; fi; `
    : "";
  const script = [
...
function windowsRemoteCommand(
  ...
  if (privilege === "admin") {
    throw new UniversalBrokerError(
      "ADMIN_UNAVAILABLE",
      `Administrator execution for Windows target ${target.id} requires the Phase 5 privileged helper.`,
    );
  }
  const inner = Buffer.from(command, "utf16le").toString("base64");
  const escapedCwd = cwd.replaceAll("'", "''");
  const script = [
    "$ErrorActionPreference = 'Stop'",
    `try { Set-Location -LiteralPath '${escapedCwd}' } catch { [Console]::Error.WriteLine('${markers.cwdRejectedMarker}'); exit 46 }`,
    `[Console]::Error.WriteLine('${markers.dispatchedMarker}')`,
    `& powershell -NoProfile -NonInteractive -EncodedCommand ${inner}`,
    "$rc = if ($null -eq $LASTEXITCODE) { 0 } else { $LASTEXITCODE }",
    `[Console]::Error.WriteLine('${markers.completedPrefix}' + $rc)`,
    "exit 0",
  ].join("; ");
...
function assertPrivilege(target: TargetDefinition, privilege: ExecutionPrivilege): void {
  if (privilege === "user") return;
  switch (target.privilege.admin.mode) {
    ...
    case "helper":
      throw new UniversalBrokerError(
        "CAPABILITY_UNAVAILABLE",
        `Administrator helper execution is reserved for Phase 5 on target ${target.id}.`,
        { evidence: { targetId: target.id, adminMode: "helper" } },
      );
    case "unavailable":
      throw new UniversalBrokerError(
        "ADMIN_UNAVAILABLE",
        `Administrator execution is unavailable on target ${target.id}.`,
        { evidence: { targetId: target.id, adminMode: "unavailable" } },
      );
  }
}
