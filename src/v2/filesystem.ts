...
export class UniversalFilesystemService {
  ...
  async execute(input: UniversalFilesystemInput): Promise<Record<string, unknown>> {
    try {
      ...
      if (resolved.privilege === "admin" && resolved.target.transport === "local") {
        throw new UniversalBrokerError(
          "ADMIN_UNAVAILABLE",
          "Local administrator filesystem access requires the privileged helper, which is installed in Phase 5.",
          { evidence: { targetId: resolved.target.id, operation: input.operation } },
        );
      }
      if (resolved.target.transport === "ssh" && resolved.target.platform === "windows") {
        throw new UniversalBrokerError(
          "CAPABILITY_UNAVAILABLE",
          "The Phase 4 SSH filesystem adapter currently supports POSIX targets; Windows OpenSSH/SFTP support remains scheduled.",
          { evidence: { targetId: resolved.target.id, platform: resolved.target.platform } },
        );
      }
  ...
  private async executeRemote(
    ...
    const path = resolved.path;
    const privilege = resolved.privilege;
    switch (input.operation) {
      case "stat":
      ...
      case "write": {
        ...
        try {
          ...
          return await this.publishRemoteFile(target, destination, staged, options, privilege);
        } finally {
      ...
      case "patch":
        if (privilege === "admin") {
          throw new UniversalBrokerError(
            "CAPABILITY_UNAVAILABLE",
            "Remote administrator patching is deferred to the privileged filesystem helper; use admin read/write with a precondition meanwhile.",
          );
        }
  ...
  private async resolveRequest(
    ...
    const resolvePath = (value: string): string => target.transport === "local"
      ...
      : resolveRemotePath(value, context?.root ?? target.defaultCwd ?? "~");
    return {
      target,
      privilege: input.privilege ?? "user",
  ...
  private async remoteRequest(
    ...
    let result = await this.execution.execute({
      ...
      privilege,
      command: `python3 -c ${shellQuote(REMOTE_FILESYSTEM_HELPER_SOURCE)} ${shellQuote(encoded)}`,
      mode: "foreground",
      yieldMs: 30_000,
    ...
    const marker = result.output.lastIndexOf(REMOTE_FILESYSTEM_RESULT_MARKER);
    if (marker < 0) {
      throw new UniversalBrokerError(
    ...
    const framed = result.output
      .slice(marker + REMOTE_FILESYSTEM_RESULT_MARKER.length)
      .trim()
      .split(/\r?\n/, 1)[0] ?? "";
  ...
  private async publishRemoteFile(
    ...
    const temporary = posix.join(
      posix.dirname(resolvedPath),
      `.devspace-v2-${posix.basename(resolvedPath)}-${randomUUID()}.tmp`,
    );
    try {
  ...
  private async remotePatch(
    ...
    validateSingleFilePatch(patch, basename(posix.normalize(path)));
    const directory = await mkdtemp(join(tmpdir(), "devspace-v2-fs-patch-"));
    const staged = join(directory, basename(posix.normalize(path)));
    try {
...
async function runSftpTransfer(
  ...
  await mkdir(options.sshControlDir, { recursive: true, mode: 0o700 });
  ...
  const args = [
    ...
    "-o", "ControlPersist=90",
    "-o", `ControlPath=${join(options.sshControlDir, "%C")}`,
    input.target.sshHost,
  ];
  const command = operation === "put"
    ? `put ${sftpQuote(input.localPath)} ${sftpQuote(input.remotePath)}\n`
    : `get ${sftpQuote(input.remotePath)} ${sftpQuote(input.localPath)}\n`;
...
function resolveRemotePath(value: string, base: string): string {
