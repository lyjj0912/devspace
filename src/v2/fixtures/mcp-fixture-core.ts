...
export function createFixtureMcpServer(): McpServer {
  ...
  server.registerTool(
    "read_environment",
    {
      title: "Read fixture environment",
      description: "Return one environment variable for environment-profile integration tests.",
      inputSchema: { name: z.string() },
      annotations: { readOnlyHint: true, destructiveHint: false },
    },
    async ({ name }) => ({
      content: [{ type: "text", text: process.env[name] ?? "" }],
      structuredContent: { name, value: process.env[name] ?? null },
    }),
  );
