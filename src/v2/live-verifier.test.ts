...
test("live verifier defaults to parallel v2 and creates only a temporary user token", async (t) => {
  ...
  const fullLoadEnd = upgradeSource.indexOf('\n)\nvalidate_release_report "$PRE_STAGE_VERIFY_REPORT" pre-stage', fullLoadStart);
  ...
  assert.match(source, /operation: "restart_broker"/u);
  assert.match(source, /process\.restart_broker response state is/u);
  assert.match(source, /operation: "restart_status"/u);
  assert.match(source, /restart_status reused a pre-restart MCP session/u);
  assert.match(source, /createBoundedSelfRestartEvidence/u);
  assert.match(source, /--exercise-self-restart requires --public-health-url/u);
  assert.match(source, /--exercise-self-restart requires --self-restart-evidence/u);
  assert.match(source, /post-restart local health status/u);
  assert.match(source, /post-restart public health status/u);
  assert.match(source, /public health identity mismatch for startedAt/u);
  ...
  assert.match(fullLoadSource, /npm run release:verify -- --mode=pre-stage --require-clean/u);
  assert.doesNotMatch(fullLoadSource, /DEVSPACE_REV3_NFR_PUBLIC_BASE_URL/u);
  assert.match(upgradeSource, /validate_release_report "\$PRE_STAGE_VERIFY_REPORT" pre-stage/u);
  assert.match(upgradeSource, /validate_release_report "\$RELEASE_VERIFY_REPORT" release/u);
  assert.match(upgradeSource, /value\.get\("status"\) == "PRE_STAGE_PASS"[\s\S]*revision3Nfr[\s\S]*"NOT_RUN"/u);
  const candidateStart = upgradeSource.indexOf('run_pm2_with_environment_file "$CANDIDATE_ENV"');
  const liveRestart = upgradeSource.indexOf('--exercise-self-restart', candidateStart);
  const finalNfr = upgradeSource.indexOf('DEVSPACE_REV3_NFR_PUBLIC_BASE_URL="$CANDIDATE_PUBLIC_BASE_URL"', liveRestart);
  const candidateCleanup = upgradeSource.indexOf("\ncleanup_candidate_before_cutover\n", finalNfr);
  assert.ok(candidateStart > fullLoadEnd, "candidate must start after the non-final pre-stage gate");
  assert.ok(liveRestart > candidateStart, "self-restart must run against the started candidate");
  assert.ok(finalNfr > liveRestart, "release NFR must run after durable self-restart evidence");
  assert.ok(candidateCleanup > finalNfr, "candidate cleanup must happen after the release NFR");
  assert.match(upgradeSource, /--candidate-public-base-url is required; production public health cannot prove candidate identity/u);
  assert.match(upgradeSource, /Candidate public origin must be isolated from production/u);
  assert.match(upgradeSource, /--token-resource "\$\{CANDIDATE_PUBLIC_BASE_URL\}\/mcp"/u);
  assert.match(upgradeSource, /DEVSPACE_REV3_NFR_SELF_RESTART_EVIDENCE="\$SELF_RESTART_EVIDENCE"/u);
  assert.match(upgradeSource, /--runtime-root "\$SOURCE"[\s\S]*release-nfr-source-runtime-tree\.json/u);
  assert.match(upgradeSource, /candidate-local-gateway-post-nfr\.json[\s\S]*post-NFR public health does not resolve to the running candidate/u);
  assert.match(upgradeSource, /mismatches=\[key for key in keys if local\.get\(key\) != public\.get\(key\)\]/u);
  assert.match(upgradeSource, /"runtimeRevision",[\s\S]*"startedAt",/u);
  assert.match(upgradeSource, /verify-runtime-tree[\s\S]*candidate-runtime-tree-post-nfr\.json/u);
